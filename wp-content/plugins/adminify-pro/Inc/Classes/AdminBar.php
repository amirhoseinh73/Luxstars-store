...
class AdminBar extends AdminSettingsModel
{
    ...
    public function __construct()
    {
        ...
        if (empty($adminify_ui)) {
            $this->jltwp_adminify_default_ui_admin_bar();
            return;
        }
        ...
        if (is_admin()) {
            ...
            add_action('wp_before_admin_bar_render', [$this, 'jltma_adminify_modify_admin_bar_menus'], 0);
    ...
    public function jltwp_adminify_default_ui_admin_bar()
    {
        if (empty($this->options['admin_bar_comments'])) {
            add_action('wp_before_admin_bar_render', [$this, 'jltwp_adminify_remove_admin_bar_comments']);
        }

        // Mode Default Admin Bar Bottom Position
        $admin_bar_position = (!empty($this->options['admin_bar_position'])) ? $this->options['admin_bar_position'] : 'top';
        if ($admin_bar_position == 'bottom') {
            add_action('admin_head', [$this, 'jltwp_adminify_move_bottom_default_admin_bar'], 100);
            add_action('wp_head', [$this, 'jltwp_adminify_move_bottom_default_admin_bar'], 100);
        }


        if (!empty($this->options['admin_bar_howdy_text'])) {
            add_filter('admin_bar_menu', [$this, 'jltwp_adminify_replace_howdy_text'], 25);
        }
    }

    public function jltwp_adminify_replace_howdy_text($wp_admin_bar)
    {
        $admin_bar_howdy_text = !empty($this->options['admin_bar_howdy_text']) ? $this->options['admin_bar_howdy_text'] : '';
        $my_account = $wp_admin_bar->get_node('my-account');
        $newtext = str_replace('Howdy,', $admin_bar_howdy_text, $my_account->title);
    ...
    public function jltwp_adminify_move_bottom_default_admin_bar()
    {
        echo '<style type="text/css"> * html body { margin-top: 0 !important; } body.admin-bar { margin-top: -28px; padding-bottom: 28px; } body.wp-admin #footer { padding-bottom: 28px; } #wpadminbar { top: auto !important; bottom: 0; } #wpadminbar .quicklinks .ab-sub-wrapper { bottom: 28px; } #wpadminbar .quicklinks .ab-sub-wrapper ul .ab-sub-wrapper { bottom: -7px; } </style>';
    }


    function jltwp_adminify_remove_admin_bar_comments()
    {
        global $wp_admin_bar;
    ...
    public function jltwp_adminify_admin_bar_css()
    {
        ...
        if (!empty($this->options['admin_bar_text_color'])) {
            $output_css .= '.adminify-top_bar nav.navbar .navbar-brand .navbar-item .wp-adminify-site-name, .adminify-top_bar nav.navbar .navbar-menu .wp-adminify-user-site-list button { color:' . $this->options['admin_bar_text_color'] . '  !important; }';
            $output_css .= '.wp-adminify #wpadminbar #wp-admin-bar-root-default li > .ab-item { color:' . $this->options['admin_bar_text_color'] . '  !important; }';
            $output_css .= '.wp-adminify #wpadminbar #wp-admin-bar-root-default li > .ab-item .ab-icon:before { color:' . $this->options['admin_bar_text_color'] . '  !important; }';
            $output_css .= '.wp-adminify #wpadminbar #wp-admin-bar-root-default li > .ab-item .ab-label { color:' . $this->options['admin_bar_text_color'] . '  !important; }';
        }

        // Icon Color
        if (!empty($this->options['admin_bar_icon_color'])) {
            $output_css .= '.adminify-top_bar nav.navbar .navbar-menu .topbar-icon svg path { fill:' . $this->options['admin_bar_icon_color'] . '  !important;}';
            $output_css .= '.adminify-top_bar nav.navbar .navbar-end i, .adminify-top_bar nav.navbar .navbar-burger i { color:' . $this->options['admin_bar_icon_color'] . '  !important;}';
        }
            ...
            <nav class="navbar adminify-top-navbar <?php echo esc_attr($admin_bar_position); ?>">
                ...
                <div id="wp-adminify-top-adminbar" class="navbar-menu">
                    <div class="navbar-start">
                        ...
                        if (isset($this->options['admin_bar_search']) && !empty($this->options['admin_bar_search'])) { ?>
...
<?php
    ...
    public function jltma_adminify_modify_admin_bar_menus()
    {
        global $wp_admin_bar;
        ...
        $wp_admin_bar->remove_menu('wp-adminify-settings-4'); // Notification Page
        $wp_admin_bar->remove_menu('wp-adminify-settings-6'); // Login Customizer
        $wp_admin_bar->remove_menu('wp-adminify-settings-10'); // Admin Pages
        $wp_admin_bar->remove_menu('wp-adminify-settings-11'); // Affiliation Page
        $wp_admin_bar->remove_menu('wp-adminify-settings-12'); // Account Page
        $wp_admin_bar->remove_menu('wp-adminify-settings-13'); // Contact Page
        $wp_admin_bar->remove_menu('wp-adminify-settings-14'); // Pricing Page
        $wp_admin_bar->remove_menu('wp-adminify-settings-16'); // Pricing Page

        // Add/Modify Admin bar links
        ...
        $wp_admin_bar->add_menu(array(
            ...
            'href'   => 'customize.php?autofocus%5Bpanel%5D=jltwp_notification_bar_panel',
        ));
        ...
        $wp_admin_bar->add_menu(array(
            ...
            'href'   => 'customize.php?autofocus[panel]=jltwp_adminify_panel',
        ));

        if (jltwp_adminify()->can_use_premium_code__premium_only()) {
            // Admin Pages
            $wp_admin_bar->add_menu(array(
                'id'     => 'wp-adminify-settings-10',
                'parent' => 'wp-adminify-settings',
                'title'  => __('Admin Pages', WP_ADMINIFY_TD),
                'href'   => 'edit.php?post_type=adminify_admin_page',
            ));
        }
    }
}
