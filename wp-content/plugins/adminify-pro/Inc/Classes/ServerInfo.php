...
class ServerInfo
{
    ...
    public function get_wp_memory_limit()
    {
        ...
        if (@ini_get('memory_limit') == '-1') {
            $memory_limit = '-1 / ' . esc_html__('Unlimited', WP_ADMINIFY_TD) . ' (' . (int)WP_MEMORY_LIMIT . ' MB)';
        }
        ...
        if ((int)WP_MEMORY_LIMIT < (int)@ini_get('memory_limit') && WP_MEMORY_LIMIT != '-1' || (int)WP_MEMORY_LIMIT < (int)@ini_get('memory_limit') && @ini_get('memory_limit') != '-1') {
            $memory_limit .= ' <span class="warning"><span class="dashicons dashicons-warning"></span> ' . sprintf(__('The WP PHP Memory Limit is less than the %s Server PHP Memory Limit', WP_ADMINIFY_TD), (int)@ini_get('memory_limit') . ' MB') . '!</span>';
        }
    ...
    public function get_php_version()
    {
        ...
        if ($php_version != 'N/A' && version_compare($php_version, '7.3', '<')) {
            $php_version = '<span class="warning"><span class="dashicons dashicons-warning"></span> ' . sprintf(__('%s - Recommend  PHP version of 7.3. See: %s', WP_ADMINIFY_TD), esc_html($php_version), '<a href="https://wordpress.org/about/requirements/" target="_blank" rel="noopener">' . __('WordPress Requirements', WP_ADMINIFY_TD) . '</a>') . '</span>';
        }
    ...
    public function get_mysql_version()
    {
        ...
        if (preg_match('/\d+(?:\.\d+)+/', $db_version_dump, $matches)) {
            ...
            $db_version = __('N/A', WP_ADMINIFY_TD);
        }
        return $db_version;
    }
    ...
    public function get_db_software()
    {
        ...
        if (!empty($db_software_dump)) {
            ...
            $db_software = __('N/A', WP_ADMINIFY_TD);
        }
    ...
    public function get_server_uptime()
    {
        ...
        if (file_exists($file)) {
            ...
            $uptime = $days . ' ' . esc_html__('Days', WP_ADMINIFY_TD) . ' ' . $hours . ' ' . esc_html__('Hours', WP_ADMINIFY_TD) . ' ' . $mins . ' ' . esc_html__('Minutes', WP_ADMINIFY_TD);
    ...
    public function format_filesize($bytes)
    {
        if (($bytes / pow(1024, 5)) > 1) {
            return number_format_i18n(($bytes / pow(1024, 5)), 0) . ' ' . __('PB', WP_ADMINIFY_TD);
        } elseif (($bytes / pow(1024, 4)) > 1) {
            return number_format_i18n(($bytes / pow(1024, 4)), 0) . ' ' . __('TB', WP_ADMINIFY_TD);
        } elseif (($bytes / pow(1024, 3)) > 1) {
            return number_format_i18n(($bytes / pow(1024, 3)), 0) . ' ' . __('GB', WP_ADMINIFY_TD);
        } elseif (($bytes / pow(1024, 2)) > 1) {
            return number_format_i18n(($bytes / pow(1024, 2)), 0) . ' ' . __('MB', WP_ADMINIFY_TD);
        } elseif ($bytes / 1024 > 1) {
            return number_format_i18n($bytes / 1024, 0) . ' ' . __('KB', WP_ADMINIFY_TD);
        } elseif ($bytes >= 0) {
            return number_format_i18n($bytes, 0) . ' ' . __('bytes', WP_ADMINIFY_TD);
        } else {
            return __('Unknown', WP_ADMINIFY_TD);
        }
    }
    ...
    public function format_filesize_kB($kiloBytes)
    {
        if (($kiloBytes / pow(1024, 4)) > 1) {
            return number_format_i18n(($kiloBytes / pow(1024, 4)), 0) . ' ' . __('PB', WP_ADMINIFY_TD);
        } elseif (($kiloBytes / pow(1024, 3)) > 1) {
            return number_format_i18n(($kiloBytes / pow(1024, 3)), 0) . ' ' . __('TB', WP_ADMINIFY_TD);
        } elseif (($kiloBytes / pow(1024, 2)) > 1) {
            return number_format_i18n(($kiloBytes / pow(1024, 2)), 0) . ' ' . __('GB', WP_ADMINIFY_TD);
        } elseif (($kiloBytes / 1024) > 1) {
            return number_format_i18n($kiloBytes / 1024, 0) . ' ' . __('MB', WP_ADMINIFY_TD);
        } elseif ($kiloBytes >= 0) {
            return number_format_i18n($kiloBytes / 1, 0) . ' ' . __('KB', WP_ADMINIFY_TD);
        } else {
            return __('Unknown', WP_ADMINIFY_TD);
        }
    }
