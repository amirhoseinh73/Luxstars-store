    ...
    class MenuEditor extends MenuEditorModel
    {
        ...
        public function adminify_save_menu_settings()
        {
            ...
            if (defined('DOING_AJAX') && DOING_AJAX && check_ajax_referer('adminify-menu-editor-security-nonce', 'security') > 0) {
                ...
                if ($options == "" || !is_array($options)) {
                    $message = __("No options supplied to save", WP_ADMINIFY_TD);
                ...
                if (is_array($options)) {
                    ...
                    $returndata['message'] = __('Settings saved', WP_ADMINIFY_TD);
                    echo json_encode($returndata);
                    die();
                } else {
                    $message = __("Something went wrong", WP_ADMINIFY_TD);
                    echo $this->ajax_error_message($message);
                    die();
                }
            }
        ...
        public function adminify_reset_menu_settings()
        {
            if (defined('DOING_AJAX') && DOING_AJAX && check_ajax_referer('adminify-menu-editor-security-nonce', 'security') > 0) {
                ...
                if (!$menu_editor_options) {
                    $returndata = array();
                    $returndata['success'] = true;
                    $returndata['message'] = __('Settings reset', WP_ADMINIFY_TD);
                    echo json_encode($returndata);
                    die();
                } else {
                    $message = __("Something went wrong", WP_ADMINIFY_TD);
        ...
        public function adminify_import_menu_settings()
        {
            if (defined('DOING_AJAX') && DOING_AJAX && check_ajax_referer('adminify-menu-editor-security-nonce', 'security') > 0) {
                ...
                echo __("Menu Imported", WP_ADMINIFY_TD);
            }
            die();
        }
        ...
        public function jltwp_adminify_menu_editor_submenu()
        {
            add_submenu_page(
                ...
                esc_html__('Menu Editor by WP Adminify', WP_ADMINIFY_TD),
                esc_html__('Menu Editor', WP_ADMINIFY_TD),
...
?>
                <script>
                    jQuery(function($) {
                        ...
                        $('.adminify-menu-settings').tokenize2({
                            placeholder: '<?php _e('Select roles or users', WP_ADMINIFY_TD) ?>'
                        });
                ...
                <div class="accordion-body adminify_top_level_settings">
                    ...
                    <div class="tab-content tab-panel panel p-4">
                        <div id="tab-<?php echo esc_attr($menu_id); ?>-1" class="tab-pane">
                            <div class="menu-editor-form">
                                <div class="columns">
                                    <div class="column">
                                        <label for="<?php echo $current_menu_item[2] ?>">
                                            <?php _e('Rename as', WP_ADMINIFY_TD); ?>
                                        </label>
                                    ...
                                    <div class="column">
                                        <label for="<?php echo $current_menu_item[2] ?>">
                                            <?php _e('Hidden For Rules', WP_ADMINIFY_TD); ?>
                                        </label>
                                        ...
                                        <div class="select is-small">
                                            <select class="adminify-menu-settings menu_setting" name="hidden_for" id="<?php echo esc_attr($menu_id); ?>-user-role-types" multiple>
                                                ...
                                                ?>
                                                <option value="Super Admin" <?php echo $sel ?>><?php _e('Super Admin', WP_ADMINIFY_TD) ?></option>
                                                <?php
                                ...
                                <div class="columns">
                                    <div class="column">
                                        <label for="">
                                            <?php _e('Change Link', WP_ADMINIFY_TD); ?>
                                        </label>
                                        <input class="menu_setting" name="link" type="url" placeholder="<?php _e('New link', WP_ADMINIFY_TD); ?>" value="<?php echo esc_url($link); ?>">
                                    </div>
                                    <div class="column">
                                        <label for="">
                                            <?php _e('Set Custom Icon', WP_ADMINIFY_TD); ?>
                                        </label>
                        ...
                        <div id="tab-<?php echo esc_attr($menu_id); ?>-2" class="tab-pane tab-pane--submenu">
                            ...
                            ?>
                                <span><?php _e('No sub menu items', WP_ADMINIFY_TD); ?></span>
                            <?php
                            } ?>
        ...
        ?>
            <div class="accordion adminify_menu_item" name="<?php echo $current_menu_item[2] ?>" id="<?php echo esc_attr($menu_id); ?>">
                ...
                <a class="menu-editor-title accordion-button p-4" href="#">
                    ...
                    <?php _e('Separator', WP_ADMINIFY_TD) ?>
                </a>
                ...
                <div class="accordion-body adminify_top_level_settings">
                    <div class="tab-content tab-panel panel p-4">
                        <div class="menu-editor-form">
                            <div class="columns">
                                <div class="column">
                                    <label for=""><?php _e('Rename as', WP_ADMINIFY_TD); ?></label>
                                    <input class="menu_setting" type="text" name="name" placeholder="<?php _e('New Name', WP_ADMINIFY_TD); ?>" value="<?php echo $name ?>">
                                </div>
                                <div class="column">
                                    <label for=""><?php _e('Hidden For Rules', WP_ADMINIFY_TD); ?></label>
                    ...
                    <div class="tab-content tab-panel panel p-4">
                        <div class="menu-editor-form">
                            <div class="columns">
                                <div class="column">
                                    <label for=""><?php _e('Rename as', WP_ADMINIFY_TD); ?></label>
                                    <input class="sub_menu_setting" type="text" data-sub-menu-id="<?php echo esc_attr($menu_id); ?>" name="name" placeholder="<?php _e('New Menu name...', WP_ADMINIFY_TD) ?>" value="<?php echo $name ?>">
                                </div>
                                <div class="column">
                                    <label for=""><?php _e('Hidden For Rules', WP_ADMINIFY_TD); ?></label>
                            ...
                            <div class="columns">
                                <div class="column">
                                    <label for=""><?php _e('Change Link', WP_ADMINIFY_TD); ?></label>
                                    <input class="sub_menu_setting" name="link" type="url" placeholder="New link" value="<?php echo $link ?>">
                                </div>
                                <div class="column">
                                    <label for=""><?php _e('Set Custom Icon', WP_ADMINIFY_TD); ?></label>
        ...
        ?>
            ...
            <p>
                <?php _e('Edit each menu item\'s name, link, icon and visibility. Drag and drop to rearange the menu. Changes will take effect after page refresh.', WP_ADMINIFY_TD); ?>
            </p>
            <p>
                ...
                <?php _e('If you have WP Adminify Menu Module disabled, icons and label dividers won\'t change.', WP_ADMINIFY_TD); ?>
