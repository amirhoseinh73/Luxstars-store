...
?>
    <div class="wrap">
        ...
        <h1>
            <?php echo Utils::admin_page_title(esc_html__('PHP_INI_Details File', WP_ADMINIFY_TD)); ?>
        </h1>

        <p style="color: #ce2754; font-weight: bold">
            <?php $name = 'php.ini';
            printf(wp_kses_post(__('For security reasons you can only read the %1$s file! Please connect to your server and modify the file in a file editor.', WP_ADMINIFY_TD)), $name); ?>
        </p>

        <p>
            <strong><?php esc_html_e('To affect your custom configuration in the php.ini file, you have to save the file in the "../wp-admin" folder.', WP_ADMINIFY_TD); ?></strong>
        </p>

        <p>
            <?php esc_html_e("The PHP configuration file, php.ini, is the final and most immediate way to affect PHP's functionality. The php.ini file is read each time PHP is initialized.", WP_ADMINIFY_TD); ?>
            <br>
            <a href="https://www.php.net/manual/en/ini.core.php" target="_blank" rel="noopener"><?php esc_html_e('Learn more about the php.ini file.', WP_ADMINIFY_TD); ?></a>
        </p>
    </div>
        ...
        if ($file) { ?>
            <p>
                <?php echo esc_html__('php.ini file was found at:', WP_ADMINIFY_TD) . ' <strong>' . esc_html($file) . '</strong>'; ?>
                <br>
            </p>
        <?php } ?>
...
<?php
    ...
    public function jltwp_adminify_php_ini_file_content($file)
    {
        ...
        $content = esc_html__('php.ini file not found!', WP_ADMINIFY_TD);
