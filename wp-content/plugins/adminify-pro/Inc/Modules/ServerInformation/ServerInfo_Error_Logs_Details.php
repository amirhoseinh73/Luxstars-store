...
class ServerInfo_Error_Logs_Details
{
    ...
    <div class="wrap">
        <h1>
            <?php echo Utils::admin_page_title(esc_html__('Error Logs', WP_ADMINIFY_TD)); ?>
        </h1>
        <?php if ((defined('WP_DEBUG') && !WP_DEBUG) || (defined('WP_DEBUG_LOG') && !WP_DEBUG_LOG)) { ?>
            <p style="color: #ce2754; font-weight: bold">
                <?php echo esc_html__('You have to set and enable "WP_DEBUG" and "WP_DEBUG_LOG" in your "wp-config.php" file, to show the error log here!', WP_ADMINIFY_TD); ?>
            </p>
        <?php } ?>

        <p>
            <?php esc_html_e('Note that debugging (the process of finding and resolving issues in software) is disabled by default in WordPress and should not be enabled on live websites. However it can be enabled temporarily to troubleshoot issues.', WP_ADMINIFY_TD); ?>
            <br>
            <a href="https://wordpress.org/support/article/debugging-in-wordpress/" target="_blank" rel="noopener"><?php esc_html_e('Learn more about debugging in WordPress.', WP_ADMINIFY_TD); ?></a>
        </p>
    </div>
                    ...
                    <li class="is-inline-block mr-4">
                        <span>
                            ...
                            <?php if ($debug_mode === 'WP_DEBUG_LOG' && $custom_file_path) { ?>
                                <span class="field-status"><?php echo esc_html__('Custom path', WP_ADMINIFY_TD); ?></span>
                            <?php } ?>
                        </span>
                    </li>
                <?php } ?>
        ...
        <div class="buttons-group is-pulled-right">
            <button id="adminify_error_log_clear" class="button clear-button">
                <?php echo esc_html__('Clear file content', WP_ADMINIFY_TD); ?>
            </button>

            <button id="adminify_error_log_refresh" class="button button-primary ml-3">
                <?php echo esc_html__('Refresh', WP_ADMINIFY_TD); ?>
            </button>
        </div>

        <p class="mt-4">
            <?php echo esc_html__('Debug log file was found at:', WP_ADMINIFY_TD) . ' <strong>' . esc_html($file) . '</strong>'; ?>
            <br>
        </p>
...
<?php
    ...
    public static function jltwp_adminify_error_log_content($file)
    {
        ...
        $content = esc_html__('debug.log file not found!', WP_ADMINIFY_TD);
