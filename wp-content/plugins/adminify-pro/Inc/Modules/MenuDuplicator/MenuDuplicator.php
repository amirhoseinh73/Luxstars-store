...
class MenuDuplicator
{
    ...
    public function jltwp_adminify_add_duplicate_button()
    {
        ...
        if ($current_screen->id == 'nav-menus' && $_GET['menu'] != '0') {
            $return = '';
            $return .= '<div style="display:none;" class="AdminifyDuplicateMenuSpinenr spinner is-active"></div><a class="AdminifyDuplicateMenuClick button button-primary button-large menu-save" href="?adminify-menu-duplicate=' . $current_menu . '">' . __('Duplicate Menu', WP_ADMINIFY_TD) . '</a>';
?>
    ...
    public function jltwp_adminify_creating_menu_duplicate()
    {
        ...
        if ($current_screen->id == 'nav-menus') {
            if (isset($_GET['adminify-menu-duplicate'])) {
                ...
                $duplicate = $this->render_menu_duplicate($_GET['adminify-menu-duplicate'], $source->name . ' ' . __('(Copy)', WP_ADMINIFY_TD));
                if ($duplicate) {
            ?>
                    <script type="text/javascript">
    ...
    public function render_menu_duplicate($id = null, $name = null)
    {
        ...
        if (!$menu_exists) {
            ...
            return $new_id = $this->render_menu_duplicate($id, $name . ' ' . __('(Copy)', WP_ADMINIFY_TD));
        }
