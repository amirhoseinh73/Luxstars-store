    ...
    class Dismiss_Admin_Notices extends AdminSettingsModel
    {
        ...
        public function __construct()
        {
            ...
            if (is_admin()) {
                ...
                if (is_multisite()) {
                    add_action('network_admin_menu',  array($this, 'adminify_dashboard_menu'));
                } else {
                    add_action('admin_menu',  array($this, 'adminify_dashboard_menu'));
                }
...
?>
        ...
        {
            ob_start();
            add_action('all_admin_notices', [$this, 'jltwp_adminify_store_notices'], 99999999);
        }
        ...
        ?>
            ...
            <div class="wrap">
                <div class="wp-adminify-notices">
                    ...
                    ?>
                    <h3>
                        <?php _e('New Notices', WP_ADMINIFY_TD); ?>
                    </h3>
                    <p>
                        <?php _e('Notices generated by date and time when it\'s been created, you can dismiss them', WP_ADMINIFY_TD); ?>
                    </p>
                    <?php
                    $iNew = 0;
                    foreach ($aStored as $key => $aNotice) {
                        ...
                        $iNew++;
                        $sDismiss = (!isset($aNotice['dismissed']) || $aNotice['dismissed'] == false) ? " - <a href='" . wp_nonce_url($sAdminURL, 'wp_adminify_notice_dismiss') . "&dismiss=" . $aNotice['reversed_key'] . "'>" . __('Dismiss Now', WP_ADMINIFY_TD) . "</a>" : false;

                        echo '<div class="wp-adminify-notice-container">';
                    ?>
                        <p>
                            <?php if ($sDismiss) : ?><strong><?php endif; ?>
                                <?php if (isset($aNotice['time'])) {
                                    echo date('Y-m-d h:m:s', $aNotice['time']);
                                } ?>
                                <?php if ($sDismiss) : ?></strong><?php endif; ?>
                            <?php echo $sDismiss; ?>
                        </p>
                    <?php if (isset($aNotice['html'])) {
                            echo $aNotice['html'];
                        }
                        echo '</div>';
                    }

                    if ($iNew == 0) _e('There\'s no new notices.', WP_ADMINIFY_TD);
                    ?>


                    <h3 style="padding-top:30px;">
                        <?php _e('Dismissed Notices', WP_ADMINIFY_TD); ?>
                    </h3>
                    ...
                    if ($iViewed == 0) _e('No dismissed notices.', WP_ADMINIFY_TD)
                    ?>
                </div>
            </div>
<?php
