...
class Utils
{
    ...
    public static function admin_page_title($title = '')
    {
        ...
        if (is_multisite()) {
            $text = ' | ' . esc_html__('Current Blog ID', WP_ADMINIFY_TD) . ': ' . get_current_blog_id(); ?>
    ...
    public static function adminfiy_help_urls($module_name = '', $docs = '', $youtube = '', $facebook_grp = '', $support = '')
    {
        ...
        $help_content = sprintf(
            __('%1$s <a class="adminify-docs-url" href="%2$s" target="_blank"> ' . Utils::docs_icon() . ' Docs</a>
                <a  class="adminify-video-url" href="%3$s" target="_blank">' . Utils::video_tutorials_icon() . ' Video Tutorial</a> <a  class="adminify-fbgroup-url" href="%4$s" target="_blank">' . Utils::fbgroup_icon() . ' Facebook Group</a> <a  class="adminify-support-url" href="%5$s" target="_blank">' . Utils::support_icon() . ' Support</a>', WP_ADMINIFY_TD),
            $module_name,
            $docs,
            $youtube,
            $facebook_grp,
    ...
    public static function adminify_upgrade_pro($custom_message = '')
    {
        ...
        if (!jltwp_adminify()->can_use_premium_code__premium_only()) {
            $jltwp_pro_notice = sprintf(__('%s <a href="https://wpadminify.com/pricing" target="_blank">Upgrade to Pro Now!</a>', WP_ADMINIFY_TD), $pro_message);
    ...
    ?>
        <div class="wp-adminify-white-label-notice-content">
            ...
            <h2>
                <?php _e('Upgrade <span>Pro</span> for White Labeling', WP_ADMINIFY_TD); ?>
            </h2>
            <p>
                <?php echo sprintf(__('<strong>%1$s</strong> can be completely re-branded with your own brand Logo, Name and Author Details. Your clients will never know what tools you are using to build their website and will think that this is your own tool set. White-labeling works as long as your license is active. ', WP_ADMINIFY_TD), WP_ADMINIFY);

                _e('', WP_ADMINIFY_TD); ?><br>
                <em><?php _e('Note: Agency Plans Only', WP_ADMINIFY_TD); ?></em>
            </p>
            <a class="wp-adminify-btn wp-adminify-get-pro" href="<?php echo esc_url('https://wpadminify.com/pricing/'); ?>" target="_blank"><?php _e('Upgrade Now', WP_ADMINIFY_TD); ?></a>
        </div>
<?php }
    ...
    public static function get_page_template_options($type = '')
    {
        ...
        $options[-1]   = __('Select', WP_ADMINIFY_TD);
        ...
        if (count($page_templates)) {
            ...
            $options['no_template'] = __('No saved templates found!', WP_ADMINIFY_TD);
        }
