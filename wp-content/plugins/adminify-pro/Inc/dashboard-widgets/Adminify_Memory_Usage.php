...
class Adminify_Memory_Usage
{
	...
	public function __construct()
	{
		...
		add_action('admin_enqueue_scripts', [$this, 'jltwp_adminify_memory_usage_admin_css']);
	}
	...
	public function jltwp_adminify_memory_usage_widget()
	{
		add_meta_box(
			...
			__('WP Memory Usage - Adminify', WP_ADMINIFY_TD),
	...
	public function jltwp_adminify_memory_usage_widget_details()
	{
		...
		$topitems = array(
			'PHP Version' => $phpversion . ' ' . (PHP_INT_SIZE * 8) . ' ' . __('Bit OS', WP_ADMINIFY_TD),
			'Memory'      => size_format($memory_usage, 2) . __(' of ', WP_ADMINIFY_TD) . $memory_limit,
		...
		if (strpos(WP_CONTENT_DIR, ABSPATH) !== false) {
			...
			$topitems['Entire Site'] = $this->jltwp_adminify_memory_usage_dir_size_display([$this->jltwp_adminify_memory_usage_dir_size(ABSPATH)]);
		} else {
			// WP_CONTENT_DIR outside ABSPATH
			$topitems['Entire Site'] = $this->jltwp_adminify_memory_usage_dir_size_display([$this->jltwp_adminify_memory_usage_dir_size(ABSPATH), $this->jltwp_adminify_memory_usage_dir_size(WP_CONTENT_DIR)]);
		}
	...
	<p><span class="spacedark">' . __('Memory Used by ', WP_ADMINIFY_TD) . '</span></p>';
		...
		foreach ($contents as $name => $value) {
			...
			$name = __($name, WP_ADMINIFY_TD); // Make translatable
			if (false === (get_transient($value)))
				echo '<span class="spacedark">' . $name . '</span>: ' . $this->jltwp_adminify_memory_usage_dir_size_display([$this->jltwp_adminify_memory_usage_dir_size($value)]) . '<br />';
			else
		...
		echo '<div class="halfspace">
		<p><span class="spacedark">Other WP Folders</span></p>';
	...
	public function jltwp_adminify_memory_usage_dir_size_display($sizes = [])
	{
		...
		if (!empty($found_error)) {
			if ($found_error == 'error_not_readable') {
				return __('Failed to read', WP_ADMINIFY_TD);
			}
		}
