...
class Adminify_Server_Uptime
{
    ...
    public function jltwp_adminify_server_uptime()
    {
        wp_add_dashboard_widget(
            ...
            esc_html__('Real-time Server Details - Adminify', WP_ADMINIFY_TD),
    ...
    public function adminify_live_server_stats()
    {
        if (defined('DOING_AJAX') && DOING_AJAX && check_ajax_referer('adminify-server-uptime-nonce', 'security') > 0) {
            ...
            if ($this->server_info->is_shell_enable()) {
                ...
                // $server_uptime             = trim(shell_exec("cut -d. -f1 /proc/uptime"));
                if (function_exists('shell_exec')) {
                    $data = shell_exec('uptime');
                    $uptime = explode(' up ', $data);
                    $uptime = explode(',', $uptime[1]);
                    // $uptime = $uptime[0] .', '.$uptime[1];
                    // $uptime = str_replace(' days', '', $uptime[0]) .','.$uptime[1];
                    $hours = explode(':', $uptime[1])[0];
                    $minutes = explode(':', $uptime[1])[1];
                }


                $datetime = time();
                $seconds_of_days = $uptime[0] * 24 * 60 * 60;
                $seconds_of_hours = $hours * 60 * 60;
                $seconds_of_minutes = $minutes * 60;

                $datetime_diff = $datetime - $seconds_of_days - $seconds_of_hours - $seconds_of_minutes;
                $server_uptime_date  = date('M j, Y H:i:s', $datetime_diff);
        ...
        <div class="adminify-server-right-now">
            <div class="columns mb-5">
                <div class="column">
                    <div class="adminify-server-right-now has-text-centered mb-2">
                        <h4 class="adminify-title">
                            <?php echo esc_html__('RAM Uses: ', WP_ADMINIFY_TD); ?>
                        </h4>
                ...
                <div class="column">
                    <div class="adminify-server-right-now has-text-centered mb-2">
                        <h4 class="adminify-title">
                            <?php echo esc_html__('CPU Load: ', WP_ADMINIFY_TD); ?>
                        </h4>
            ...
            <div class="columns">
                <div class="column">
                    <div class="adminify-server-right-now has-text-centered mb-2">
                        <h4 class="adminify-title">
                            <?php echo esc_html__('PHP Memory Uses: ', WP_ADMINIFY_TD); ?>
                        </h4>
                ...
                <div class="column">
                    <div class="adminify-server-right-now has-text-centered mb-2">
                        <h4 class="adminify-title"><?php echo esc_html__('WP Memory Uses: ', WP_ADMINIFY_TD); ?></h4>
            ...
            <div class="adminify-server-uptime has-text-centered" id="adminify-server-uptime">
                <h4 class="adminify-title">
                    <?php echo esc_html__('Server Uptime: ', WP_ADMINIFY_TD); ?>
                </h4>
