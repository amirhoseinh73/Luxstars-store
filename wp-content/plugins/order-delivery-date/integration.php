...
class orddd_integration {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_form_field_select', array( $this, 'orddd_location_field' ), 20, 4 );
		 ...
		 // Adds delivery date, time and pickup locations in pdf invoice which is generated by the WooCommerce Germanized plugin.
		 add_action( 'storeabill_after_invoice_content', array( &$this, 'orddd_add_delivery_fields_to_pdf_invoice' ), 10, 1 );
	}
	...
	/**
	 * Adds delivery date, time and pickup locations in pdf invoice generated by the WooCommerce Germanized plugin.
	 *
	 * @hook storeabill_after_invoice_content
	 * @param string $invoice_id - invoice id
	 */
	function orddd_add_delivery_fields_to_pdf_invoice( $invoice_id ) {    
		if ( ! function_exists('sab_get_document') || ! $invoice_id ) {
			return;
		}
		$get_document     = sab_get_document( $invoice_id );
		$order_id         = $get_document->get_reference_id();
		$data             = get_post_meta( $order_id );
		$order_date       = orddd_common::orddd_get_order_delivery_date( $order_id );
		$order_time       = orddd_common::orddd_get_order_timeslot( $order_id );
		...
		$extra_fields     = array();
		if ( $address ) {
			$extra_fields[] = $locations_label . ': ' . $address;
		}
		if ( $order_date ) {
			$extra_fields[] = $date_field_label . ': ' . $order_date;
		}
		if ( $order_time ) {
			$extra_fields[] = $time_field_label . ': ' . $order_time;
		}
		if( ! empty( $extra_fields ) ) {
			echo '<div style="margin-top: 50px;">' . implode( "</div><div>", $extra_fields ) . '</div>';
		}
	}
}
